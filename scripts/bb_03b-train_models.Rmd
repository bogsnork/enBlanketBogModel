...
subset predictors (depends on model)
```{r}
predictors <- raster::subset(predictors.all, c("elev", "aspect", "slope",
                             "outflow", "inflow", "gdd", "gsl",
                             "rain_ann", "raindays_10mm",
                             "temp_min", "temp_max"))
```
...
```{r}
myBiomodOption <- BIOMOD_ModelingOptions(RF = list(maxnodes = 40))
```

# train the model

```{r}
myBiomodModelOut <- BIOMOD_Modeling(myBiomodData, models = c("RF", "GLM"), 
...
```{r}
mod.plot<-BIOMOD_LoadModels(myBiomodModelOut,models=c('RF','GLM'))
strip.plots<-response.plot2(models=mod.plot[c(2,4)],
...
myBiomodProj <- BIOMOD_Projection(myBiomodModelOut, new.env = predictors, proj.name = "current", 
                                  selected.models = "all", binary.meth = "ROC", build.clamping.mask = FALSE)
```
...
```{r}
list.files(path = "../scripts/blanket.bog/proj_current", pattern = "gri")
```

```{r}
myBiomodProj@proj@val@layers
plot(myBiomodProj@proj@val@layers[[2]])
```


```{r}
#write GLM to file
writeRaster(myBiomodProj@proj@val@layers[[2]], filename = "../outputs/bb_GLM_mod1b.tif", overwrite = TRUE)
```
