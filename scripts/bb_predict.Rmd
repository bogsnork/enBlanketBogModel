...
#load moorland line
moorlandSPDF <- readOGR(dsn = "../data/Moorland_Line_RPA/Moorland_Line.gdb",
                        layer = "Moorland_Line", verbose = TRUE)
```
...
## Constrain data
Crop to moorland boundary

```{r}
## this is really slow: 
predictors <- raster::crop(x = predictors, y = moorlandSPDF)
#writeRaster(x = predictors, filename = "../data/predictors_moorlandline.tif")
#predictors <- raster("../data/predictors_moorlandline.tif")
#DIDN'T WORK, didn't crop anything... duh????
```
...
writeRaster(x = output, filename = "../outputs/BlanketBogRFModel3b.tif")

plot(output)
plot(moorlandSPDF, add = T)
