...
save(M.rf.2, file = "../data/models/Mrf2.rds")
...
```{r}
plot(varImp(object= M.rf.2), main= "RF - Variable Importance")
```
...
```{r}
#Predictions
predictions <- predict(object = M.rf.2, newdata = testing, type="raw")
table(predictions)
...
```{r}
M.rf.2.final.final <- train(bbog ~ .,
                data = input.data, 
                method = "rf")

save(M.rf.2.final, file = "../data/models/Mrf2final.rds")
