...
save(M.rf.5, file = "../data/models/Mrf5.rds")
...
```{r}
plot(varImp(object= M.rf.5), main= "RF - Variable Importance")
```
...
```{r}
#Predictions
predictions <- predict(object = M.rf.5, newdata = testing, type="raw")
table(predictions)
...
```{r}
M.rf.5.final <- train(bbog ~ .,
                data = input.data,
                method = "rf")

save(M.rf.5.final, file = "../data/models/Mrf5final.rds")

M.rf.5.final

M.rf.5.final$coefnames
```
