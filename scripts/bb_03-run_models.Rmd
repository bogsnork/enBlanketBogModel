...
save(M.rf.6, file = "../data/models/Mrf6.rds")
...
```{r}
plot(varImp(object= M.rf.6), main= "RF - Variable Importance")
```
...
```{r}
#Predictions
predictions <- predict(object = M.rf.6, newdata = testing, type="raw")
table(predictions)
...
```{r}
M.rf.6.final <- train(bbog ~ .,
                data = input.data,
                method = "rf")

save(M.rf.6.final, file = "../data/models/Mrf6final.rds")

M.rf.6.final

M.rf.6.final$coefnames
```
