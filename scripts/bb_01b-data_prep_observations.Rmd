...
* Create non-PHI blanket bog area for training data (if training on a specific area)
    * Erase tool
    * Erase "PHI Blanket Bog" from "England_country.shp"
    * output feature class: "England_no_phibb.shp"  
* Create additional absences
    * Create random Points tool
    * constrain by "England_no_phibb.shp"  
    * 10000 points
    * output feature class: "Eng_not_bb_random_pts.shp" 
...
not_bb_random_pts_SPDF <- readOGR(dsn = "../data/Eng_not_bb_random_pts.shp", verbose = TRUE)
England_boundary_SPDF <- readOGR(dsn = "../data/England_country.shp", verbose = TRUE)
...
observations_SPDF$CID <- NULL
```

```{r}
table(observations_SPDF$bbog)
```

## export

```{r}
writeOGR(obj = observations_SPDF, dsn = "../data/FEP_presabs.shp", driver = "ESRI Shapefile", layer = "observations_SPDF")
```
