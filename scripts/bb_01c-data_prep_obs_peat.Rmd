...
model.area <- readOGR(dsn = "../data/England_country.shp")
no_PHIBB <- readOGR(dsn = "../data/England_no_phibb.shp")
```
...
absences <-   SpatialPointsDataFrame(
  coords = spsample(x = no_PHIBB, n = 1000, type = "random"), 
  data = data.frame(bbog = rep("absent", 1000)), 
...
plot(observations, col = observations$bbog)
...
```{r}
writeOGR(obj = observations, dsn = "../data/peat_depth_presabs_Eng.shp", driver = "ESRI Shapefile", verbose = FALSE, layer = "observations")
```
